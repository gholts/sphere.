...
actor ProxyIconCache {
    ...
    nonisolated private static func loadImage(for icon: String, maxIconBytes: Int) async -> CGImage? {
        ...
        do {
            ...
            guard isSuccessful(response), let image = await decodeImageData(data, maxIconBytes: maxIconBytes) else { return nil }
    ...
    nonisolated private static func decodeDataImage(_ icon: String, maxIconBytes: Int) async -> CGImage? {
