...
nonisolated struct MihomoClient: ProxyBackendClient {
    ...
    func refreshProxyProvider(_ name: String) async throws {
        try await transport.requestNoBody(path: "/providers/proxies/\(transport.escaped(name))", method: "PUT")
    }
    ...
    func refreshRuleProvider(_ name: String) async throws {
        try await transport.requestNoBody(path: "/providers/rules/\(transport.escaped(name))", method: "PUT")
    }
    ...
    func logs(level: LogLevel) -> AsyncThrowingStream<LogEntry, Error> {
        transport.webSocketStream(path: "/logs", query: [URLQueryItem(name: "level", value: level.rawValue)], response: LogEntry.self)
    }
